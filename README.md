...
✅ 前端开发者 / 喜欢"自己能修"的工具的人（源码公开、可审计、可自部署）
...
Nestworth 源码采用 **Business Source License 1.1 (BUSL-1.1)** 授权——一种"源码可见 + 个人/内部自用永久免费 + 限制竞品托管 + N 年后自动转 AGPL"的协议。详见 [`LICENSE`](LICENSE)。

**你可以**：
- ✅ 个人、家庭、单一组织内部自用（永久免费）
- ✅ 自己 clone、fork、修改、自部署
- ✅ 阅读和审计源码（验证我们的隐私承诺）
- ✅ 每个版本发布满 4 年后，自动转为 **AGPL-3.0-or-later**，社区可任意使用

**你不能**：
- ❌ 把 Nestworth 包装成托管 SaaS / 竞品服务卖给第三方（需购买商业授权）

**历史 MIT 版本**：本仓库在 2026-05-17（commit `f6e6500`，tag `v0.2.1-final-mit`）之前的所有代码继续保持 MIT 授权，详见 [`LEGACY-MIT.md`](LEGACY-MIT.md)。

商业授权 / 合作 / 商标问询：`huoqilei.hql@alibaba-inc.com`
